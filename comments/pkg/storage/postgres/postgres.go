...
func New(url string) (*postgres, error) {
	log.Println("Connecting to comments database...")
	for {
...
func (p *postgres) AddComment(c storage.Comment) error {
	if c.NewsID == 0 {
		...
		_, err = p.db.Exec(context.Background(), "INSERT INTO comments (news_id, parent_comment_id, content) VALUES ($1, $2, $3);", newsId, c.ParentCommentID, c.Content)
		if err != nil {
			return err
		}
		return nil
	} else {
		_, err := p.db.Exec(context.Background(), "INSERT INTO comments (news_id, content) VALUES ($1, $2);", c.NewsID, c.Content)
		if err != nil {
			return err
		}
		return nil
...
func (p *postgres) DeleteComment(c storage.Comment) error {
	_, err := p.db.Exec(context.Background(), "DELETE FROM comments WHERE id = $1;", c.ID)
	if err != nil {
		return err
	}
	return nil
