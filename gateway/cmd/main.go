...
func main() {
	...
	log.Printf("Gateway server started on %s", gatewayAddr)
	err := http.ListenAndServe(gatewayAddr, api.Router())
	if err != nil {
		log.Fatal("The gateway server could not start. Error:", err)
	}
}
